...
callback: async({interaction}) => {
    ...
    if (interaction.options.getString('show')) {
        const searchResults = await notificationSchema.find({userId: interaction.user.id, Id: interaction.options.getString('show')}).sort({_id:-1})
        ...
        const searchResults = await notificationSchema.find({userId: interaction.user.id}).sort({_id:-1})
