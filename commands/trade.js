...
callback: async({interaction}) => {
    ...
    if (main === true) return
    const cldn = await functions.cooldownCheck(interaction.user.id, 'trade', 15, interaction)
    if (cldn === true) return
    ...
    const userProfileSender = await profileSchema.findOne({userId: sender.id})
    const userProfileRecipient = await profileSchema.findOne({userId: recipient.id})
    ...
    if (!userProfileSender) {
        profileSchema.create({userId: sender.id})
    ...
    if (senderAmount && recipientAmount && recipientItem && !senderItem) {
        const checkTheirInv = await invSchema.findOne({userId: recipient.id, itemId: itemFoundThem.id})
        ...
        if (checkTheirInv.amount - recipientAmount === 0) {
                ...
                if (i.customId === 'confirm-trade-sender') {
                    ...
                    collectorRecipient.on('collect', async(i) => {
                        ...
                        if (i.customId === 'confirm-trade-recipient') {
                            ...
                            const checkYourInv = await invSchema.findOne({userId: sender.id, itemId: itemFoundThem.id})
                            if (!checkYourInv) {
                                invSchema.create({userId: sender.id, itemId: itemFoundThem.id, item: itemFoundThem.name, emoji: itemFoundThem.emoji, amount: recipientAmount})
                            } else {
                            ...
                            functions.createNewNotif(sender.id, `Trade Successful with ${recipient} (They gave \`${recipientAmount.toLocaleString()}\` ${itemFoundThem.emoji} ${itemFoundThem.name} for your \`${senderAmount.toLocaleString()}\` coins)`)
                            functions.createNewNotif(recipient.tag, `Trade Successful with ${sender} (You gave \`${recipientAmount.toLocaleString()}\` ${itemFoundThem.emoji} ${itemFoundThem.name} for their \`${senderAmount.toLocaleString()}\` coins)`)
            ...
            const checkYourInv = await invSchema.findOne({userId: sender.id, itemId: itemFoundYour.id})
                    ...
                    if (i.customId === 'confirm-trade-sender') {
                        ...
                        collectorRecipient.on('collect', async(i) => {
                            ...
                            if (i.customId === 'confirm-trade-recipient') {
                                ...
                                const checkTheirInv = await invSchema.findOne({userId: recipient.id, itemId: itemFoundYour.id})
                                if (!checkTheirInv) {
                                    invSchema.create({userId: recipient.id, itemId: itemFoundYour.id, item: itemFoundYour.name, emoji: itemFoundYour.emoji, amount: senderAmount})
                                } else {
            ...
            const invCheckSender = await invSchema.findOne({userId: sender.id, itemId: itemFoundYour.id})
            const invCheckRecipient = await invSchema.findOne({userId: recipient.id, itemId: itemFoundThem.id})
                ...
                if (i.customId === 'confirm-trade-sender') {
                    ...
                    collectorRecipient.on('collect', async(i) => {
                        ...
                        if (i.customId === 'confirm-trade-recipient') {
                            ...
                            const checkTheirInv1 = await invSchema.findOne({userId: recipient.id, itemId: itemFoundYour.id})
                            if (!checkTheirInv1) {
                                invSchema.create({userId: recipient.id, itemId: itemFoundYour.id, item: itemFoundYour.name, emoji: itemFoundYour.emoji, amount: senderAmount})
                            } else {
                            ...
                            const checkYourInv1 = await invSchema.findOne({userId: sender.id, itemId: itemFoundThem.id})
                            if (!checkYourInv1) {
                                invSchema.create({userId: sender.id, itemId: itemFoundThem.id, item: itemFoundThem.name, emoji: itemFoundThem.emoji, amount: recipientAmount})
                            } else {
                            ...
                            const checkTheirInv2 = await invSchema.findOne({userId: recipient.id, itemId: itemFoundThem.id})
                            if (checkTheirInv2.amount - recipientAmount === 0) checkTheirInv2.delete()
                            else checkTheirInv2.amount -= recipientAmount; checkTheirInv2.save()

                            const checkYourInv2 = await invSchema.findOne({userId: sender.id, itemId: itemFoundYour.id})
                            if (checkYourInv2.amount - senderAmount === 0) checkYourInv2.delete()
                            else checkYourInv2.amount -= senderAmount; checkYourInv2.save()
