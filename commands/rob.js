...
callback: async({interaction}) => {
    ...
    const thiefProfile = await profileSchema.findOne({userId: interaction.user.id})
    const victimProfile = await profileSchema.findOne({userId: victim.id})
    if (!thiefProfile) {
        interaction.reply({
            embeds: [
                new MessageEmbed()
        ...
        return await profileSchema.create({userId: interaction.user.id})
    }
    ...
    if (willRobWork === 0) {
        ...
        if (cldn === true) return
        functions.createRecentCommand(interaction.user.id, `rob-success`, `USERROBBED: ${victim.id} | AMOUNT: ${takeFromVictim.toLocaleString()}`, interaction)
        ...
        const cldn = await functions.cooldownCheck(interaction.user.id, 'rob', 300, interaction)
        if (cldn === true) return
        functions.createRecentCommand(interaction.user.id, `rob-fail`, `USERROBBED: ${victim.id} | AMOUNTLOST: ${takeFromTheif.toLocaleString()}`, interaction)
