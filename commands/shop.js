...
callback: async({interaction}) => {
    ...
    if (interaction.options.getSubcommand() === 'view') {
        ...
        if (!userProfile) {
            await profileSchema.create({userId: interaction.user.id})
        ...
        const scanDatabaseForItem = await invSchema.findOne({userId: interaction.user.id, itemId: itemQuery.toLowerCase()})
        if (scanDatabaseForItem) {
            ...
            interaction.reply({
                ...
                invSchema.create({userId: interaction.user.id, emoji: itemFound.emoji, item: itemFound.name, itemId: itemFound.id, amount: amount})
                userProfile.wallet -= itemFound.buyPrice * amount
                userProfile.save()
                interaction.reply({
                    embeds: [
