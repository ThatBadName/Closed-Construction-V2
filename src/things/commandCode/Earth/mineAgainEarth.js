...
const { colours, emojis, randomItemsMine, reasonsToBreakPick } = require('../../../things/constants')
const {
    EmbedBuilder,
    ActionRowBuilder,
    ButtonBuilder
...
async function mineAgainEarth(interaction) {
    ...
    if (willGetRandomItem === 0) {
        let itemToGet = randomItemsMine[Math.floor(Math.random() * randomItemsMine.length)]
