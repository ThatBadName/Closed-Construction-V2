...
module.exports = (client) => {
   client.on('interactionCreate', async(interaction) => {
    const checkMain = await maintenance.findOne({maintenance: true})
    if (checkMain) return
    ...
    try {
        userFromId = userFromId.slice(17,36)
    } catch {}
    try {
        ctmId = ctmId.slice(0,17)
    } catch {}
    if (ctmId !== 'reply-text-modal-') return
    const userRecieveId = userFromId

    const result = await blockSchema.findOne({blockedById: interaction.user.id, blockedUserId: userRecieveId})
    ...
    const result2 = await blockSchema.findOne({blockedUserId: interaction.user.id, blockedById: userRecieveId})
    ...
    functions.createRecentCommand(interaction.user.id, 'text-reply', `USER: <@${userRecieveId}> | CONTENT: ${textMessage}`, interaction)
    ...
    const textEmbed = new MessageEmbed()
        ...
        .setDescription(`${interaction.user} (\`${interaction.user.id}\`) has replied to you:\n${textMessage}`)
        .setColor('0x545dff')
        .setTimestamp()
        .setFooter({text: `You can disable texting in the settings menu`})
    ...
    await member.send({embeds: [textEmbed], components: [textControls]}).catch(e => {
        failed = true
        return interaction.reply({embeds: [textFailedEmbed]})
    })

    if (failed === false) interaction.reply({embeds: [textConfirmEmbed]})
})
},

module.exports.config = {
   dbName: 'CREATE TEXT REPLY',
   displayName: 'Create Text Reply',
}