...
module.exports = (client) => {
   client.on('interactionCreate', async(interaction) => {
      ...
      if (interaction.customId.startsWith('block-')) {
         ...
         const check = await blockSchema.findOne({blockedUserId: blockId, blockedById: interaction.user.id})
         ...
         await blockSchema.create({blockedUserId: blockId, blockedById: interaction.user.id})
      ...
      const user = await client.users.fetch(blockId).catch(e => null)
         ...
         user.send({
            embeds: [
               ...
               .setFooter({text: `Please Note: I will not DM you if you are unblocked`})
            ]
         }).catch(err => {})
         ...
         const result = await blockSchema.findOne({blockedById: interaction.user.id, blockedUserId: replyId})
         ...
         const result2 = await blockSchema.findOne({blockedUserId: interaction.user.id, blockedById: replyId})
